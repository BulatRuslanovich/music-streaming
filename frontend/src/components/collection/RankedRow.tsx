 ...
 * цифры справа. Одна геометрия на треки с обложками и на топы статистики без них: в двух
 * копиях они расходятся, и любимые исполнители на своей же странице выглядят строчками таблицы.
 *
...
export function RankedRow<T extends string>({
  ...
  const shell = cn(
    ...
    // Обе ширины задаются в одной ветке. Отдельная мобильная строка ниже перекрыла бы
    // выбранную здесь безусловно, правая колонка на телефоне снова стала бы `auto`,
    // и полосы разъехались бы уже там.
    bar === undefined
