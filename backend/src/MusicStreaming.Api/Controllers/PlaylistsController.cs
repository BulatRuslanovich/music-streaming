...
///
/// <para>
/// Плейлист принадлежит пользователю: чужой приватный не виден вовсе (404, а не 403 — существование
/// чужого плейлиста само по себе не должно быть видно). Публичный доступен на чтение и
/// прослушивание всем вошедшим, но менять его по-прежнему может только владелец.
/// </para>
/// </summary>
[ApiController]
[Route("api/playlists")]
public class PlaylistsController(PlaylistService playlists, StreamingService streaming) : ControllerBase
{
    ...
    ///
    /// <para>
    /// Повторное добавление того же трека отсекается уникальным индексом базы, а не проверкой в
    /// коде: позиция раньше вычислялась запросом <c>MAX(position) + 1</c>, и два одновременных
    /// добавления — двойной клик или два устройства — читали одно значение и вставляли обе строки.
    /// </para>
    /// </summary>
