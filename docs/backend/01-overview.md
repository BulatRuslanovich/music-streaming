...
| **Доступ снаружи — только через обратный прокси** | Служебные ручки (`/metrics`, `/docs`, `/openapi`) защищены тем, что прокси их не пробрасывает, а не отдельной авторизацией |
